...
const coarse=matchMedia('(pointer: coarse)').matches||navigator.maxTouchPoints>0;let joyId=-1,lookId=-1,lastLX=0,lastLY=0;
...
el('start').onclick=()=>{if(!artReady)return;if(ended)reset();started=true;paused=false;running=true;el('overlay').classList.add('hidden');if(!coarse)renderer.domElement.requestPointerLock();sound(330,.25);notice('苍岚古寺 · 挑战镇山巨兽');};
